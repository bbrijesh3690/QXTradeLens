  ...
  var _tc = function () {
    ...
    function scanPair(symbol, entries, asset, nowSec, tfs) {
      ...
      for (const tf of tfs) {
        ...
        if (last && isFinite(last.t)) {
          row.age = Math.min(row.age, Math.max(0, nowSec - last.t));
        }
    ...
    function sweepTargets() {
      ...
      getPairTabs().forEach((tab) => {
        ...
        let age = Infinity;
        for (const key in entries || {}) {
          const c = entries[key] && entries[key].candles;
          if (c && c.length && isFinite(c[c.length - 1].t)) {
            age = Math.min(age, Math.max(0, now - c[c.length - 1].t));
          }
        }
    ...
    function renderSweep(panel) {
      ...
      const btn = panel.querySelector('[data-mtf="sweep"]'),
        msg = panel.querySelector(".tcScanFootMsg");
      if (!btn) {
        return;
      }
      const running = !!sweepState,
        label = running ? "Stop" : "Sweep";
      if (btn.textContent !== label) {
        btn.textContent = label;
      }
      btn.classList.toggle("tcOn", running);
      const text = running
    ...
    function createMtf() {
      ...
      t.innerHTML =
        ...
        n +
        '<div class="tcMtfScan"><div class="tcMtfScanHdr"></div><div class="tcMtfScanList"></div><div class="tcMtfScanFoot"><button type="button" class="tcMtfSweep" data-mtf="sweep" title="Visit each open pair that has fallen behind, collect its candles, and come back here">Sweep</button><span class="tcScanFootMsg"></span></div></div>' +
        ["e", "w", "n", "s", "ne", "nw", "se", "sw"]
      ...
      t.addEventListener("click", (ev) => {
        ...
        const sweepBtn = ev.target.closest('[data-mtf="sweep"]');
        if (sweepBtn) {
          ev.preventDefault();
          if (sweepState) {
            // Ends it there and then. A walk already in flight finishes its own last timeframe and puts the
            // chart back as it always does - waiting for that before letting go would make Stop feel stuck.
